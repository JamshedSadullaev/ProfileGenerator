...
<div class="header">
    <div class="p-3 mb-2 bg-danger text-white d-flex justify-content-center">
        <h1>My Team</h1>
...
const generateTemplate = function (arr) {
    let icon;
    let info;
    if (arr.title === "Manager") {
        icon = `<i class="bi bi-cup-hot"></i>`
        info = `Office number: ${arr.officeNumber}`

    } else if (arr.title === "Engineer") {
        icon = `<i class="bi bi-eyeglasses"></i>`
        info = `Github: <a href="https://www.github.com">${arr.github}</a>`
    } else if (arr.title === "Intern") {
        icon = `<i class="bi bi-mortarboard"></i>`
        info = `School:${arr.school}`
    }
    return `

        <div class="card" style="width: 18rem;">
    <div class="card-body p-3 mb-2 bg-primary text-white">
    <h4 class="card-title">${arr.name}</h4>
  <h4 class="card-title">${icon}</i> ${arr.title}</h4>
</div>
